...
void InitEUI(std::string title, int width, int heigth) 
{
	...
	EditorInstance::GetSingleton()->menuStrip->AddItem("File", "Exit", boost::bind(quick_exit, 0));
...
void EditorMenu(nk_context *ctx) 
{
	nk_menubar_begin(ctx);
	{
		...
		{
			printf("\n\n%s\n\n", rootElement->name);
			if (nk_menu_begin_label(ctx, rootElement->name.c_str(), NK_TEXT_ALIGN_LEFT, nk_vec2(110, 120))) 
			{
